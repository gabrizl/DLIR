...
def Model_DRLIR(features, labels, mode, params):
    ...
    W_p = tf.tile(tf.expand_dims(tf.compat.v1.get_variable("W_p", [params['vocabulary_length'], params['embedding_size']], dtype=tf.float32), 0),[params['batch_size'], 1, 1])
...
def conv_1d_layer(inputs, num_units):
    ...
    with tf.compat.v1.variable_scope("conv_1d_layer", reuse=tf.compat.v1.AUTO_REUSE):
