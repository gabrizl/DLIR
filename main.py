...
try:
    ...
    def updateMatrix_basedPopularity(mat):
        try:
            a = _np.array(mat, dtype=float)
            # simple row-normalization fallback
            row_sums = a.sum(axis=1)
            row_sums[row_sums == 0] = 1.0
            return (a / row_sums[:, None])
        except Exception:
            return _np.array(mat)

    def gcn_network(mat, maxPOI, h1, h2):
        # fallback: ensure square matrix of size maxPOI
        a = _np.array(mat, dtype=float)
        if a.ndim == 1:
            a = a.reshape((1, -1))
        if a.shape[0] < maxPOI or a.shape[1] < maxPOI:
            b = _np.zeros((maxPOI, maxPOI), dtype=float)
            r0 = min(a.shape[0], maxPOI)
            c0 = min(a.shape[1], maxPOI)
            b[:r0, :c0] = a[:r0, :c0]
            return b
        return a

    def padding(seqs, max_len):
        out = []
        for s in seqs:
            # handle single integers or sequences
    ...
    def makeRecentSequence(train, valid, test):
        # fallback: return inputs unchanged
        return train, valid, test

    def makeRecentSequenceOne(seq):
        out = []
        for s in seq:
            if hasattr(s, '__iter__') and not isinstance(s, (str, bytes)):
                out.append(list(s))
    ...
    def Convert_Time_ID(t_seq):
        ...
        for t in t_seq:
            try:
                dt = _pd.to_datetime(t, unit='s', errors='coerce')
                if _pd.isna(dt):
                    month = 0
                    weekday = 0
                    day = 0
                    ...
                    day = int(dt.day)
                season = (month - 1) // 3 + 1 if month > 0 else 0
            except Exception:
                month = 0
                weekday = 0
                day = 0
            ...
            days.append(day)
        return seasons, months, weekdays, days

    ut = types.SimpleNamespace(
        updateMatrix_basedPopularity=updateMatrix_basedPopularity,
        gcn_network=gcn_network,
        padding=padding,
        makeRecentSequence=makeRecentSequence,
        makeRecentSequenceOne=makeRecentSequenceOne,
        Convert_Time_ID=Convert_Time_ID
    )
import os
import shutil
...
def data_Preprocessing(data):
    ...
    dfVisits = pd.read_csv("New_Data/userVisits-" + data + "-allPOI.csv", delimiter=";",encoding='UTF-8')
...
def rideDuration(dataset, dfVisits, maxPOINumber):
    ...
    POI_data = pd.read_csv("Data/POI-"+dataset+".csv",delimiter = ";",encoding='UTF-8')
...
def train_model(train_input,train_poi_sequence_y,data, GCN):
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as session:
        ...
        classifier.train(input_fn=lambda: mdrl.train_input_fn(train_input, np.array(train_poi_sequence_y).astype('int64'), DEFINES.batch_size), steps=DEFINES.train_steps)
...
def build_queue_times(data):
    ...
    queueInfo = pd.read_csv("Data/queueTimes-" + data + ".csv",delimiter=';',encoding='UTF-8')
...
def test_Model_Itinerary(data, dfVisits,test_seq ):
    ...
    costInfo = pd.read_csv("Data/costProfCat-"+data+"-all.csv", delimiter=";",encoding='UTF-8')
